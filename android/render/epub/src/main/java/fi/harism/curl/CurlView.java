...
public class CurlView extends GLSurfaceView implements View.OnTouchListener,
	...
	public void onDrawFrame() {
		...
		if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
			...
			mAnimate = false;
			requestRender();
			// LinReads addition (Apache-2.0 permits modification; see NOTICE.txt): notify the host
	...
	/**
	 * LinReads addition (Apache-2.0; see NOTICE.txt). Sets an observer notified on the UI thread once
	 * a curl animation finishes, with the resulting current page index.
	 */
	...
	/**
	 * LinReads addition. Notified when a curl animation settles.
	 */
	public interface CurlAnimationObserver {
		void onCurlAnimationEnd(int currentIndex);
	}
