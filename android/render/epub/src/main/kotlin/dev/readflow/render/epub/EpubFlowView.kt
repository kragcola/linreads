...
internal class EpubFlowView(
    ...
    fun setModeAnchored(value: Mode, layoutOffset: Int) {
        ...
        val conversionSnapshot = if (hidePagedConversion) {
            snapshotViewport(PageShotLeaseKind.PINNED, "continuity.mode")
        } else {
            null
        }
    ...
    /** Dirty warm-shot identities follow the active turn, then remap to their settled cache slots. */
    ...
    private fun preCachePageTextures() {
        ...
        if (frontMatches && targetMatches && previousMatches) return
    ...
    private fun refreshCachedPageShotInPlace(slot: CachedPageShotSlot): InPlaceRefreshOutcome {
        ...
        if (bitmap.width != width || bitmap.height != height) return InPlaceRefreshOutcome.DROPPED_STALE
    ...
    fun releasePageShotBudgetForBoundaryPreview(forward: Boolean) {
    ...
    private fun drawPageBoundaryImagePreview(
        ...
        val imageTop = layout.getLineTop(preview.imageLine) + textView.paddingTop - pageTopPx
        val previewTopInViewport = maxOf(precedingBottom, imageTop).coerceIn(0, height)
    ...
    private fun snapshotViewport(
        ...
        allocatePageShot(width, height, kind, label) { bmp ->
    ...
    fun snapshotPageAt(topPx: Int): Bitmap? =
        snapshotPageAt(topPx, settledWindowAtTop(topPx))
     ...
     */
    private fun abortLocalPageShotTurnForExternalMutation(restoreOrigin: Boolean = false) {
        cancelPendingLocalPageShotHandoff(consumeGesture = true)
    ...
    private fun clearFlipOverlay(preserveActivePixelRefreshes: Boolean = false) {
        ...
        curlDrawable = null
        if (!preserveActivePixelRefreshes) {
            activeFlipFrontPixelRefreshPending = false
            activeFlipRevealedPixelRefreshPending = false
        }
        if (
    ...
    private fun finishBoundaryInteractiveTurn(commit: Boolean) {
        ...
        curlOrigin = null
        onBoundaryTurnDiscarded?.invoke(preview)
        releasePageShotBudgetForBoundaryPreview(preview.forward)
    }
    ...
    private fun stablePageShotCapacity(): Int {
        ...
        val bytesPerShot = width.toLong() * height.toLong() * 4L
