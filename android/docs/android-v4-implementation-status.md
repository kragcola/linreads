...
| Phase | 目标 | 状态 | 备注 |
|-------|------|------|------|
| **A 无账号本地阅读闭环** | 安装后直接打开本地书并继续阅读 | ✅ **功能就绪** | 书架+导入+阅读+进度+设置全链路通；ACTION_VIEW/ACTION_SEND 代码入口就绪；性能/TalkBack/文件入口真机 smoke 待补 |
...
- ✅ ACTION_VIEW/ACTION_SEND 入口接入本地导入链路（resolver 单测覆盖；真机 smoke 待补）
...
- ⏳ ACTION_VIEW/ACTION_SEND 真机 smoke（文件管理器打开、分享面板分享后自动导入并进入 reader）
- ⏳ 性能测量（冷启动/首屏/翻页/内存）
...
### 立即可做（Phase A 收尾）
1. **ACTION_VIEW/ACTION_SEND 真机 smoke**：从文件管理器打开、从分享面板发送 TXT/EPUB/PDF/MD，确认自动导入并进入 reader。
2. **性能测量**：冷启动/首屏/翻页/内存峰值正式测量并记录。
3. **TalkBack smoke test**：ReaderScreen chrome 元素无障碍标签验证。
4. **Calibre 端到端验证**：真机上连真实 Calibre 服务器测试搜索和下载。

### 下一阶段启动条件（Phase B+）
- [ ] Phase A 文件入口 smoke + 性能 + TalkBack 测量完成
- [ ] EPUB 分页模式 gate 决策（做/不做/推迟）
- [ ] Calibre 书源接入（连接向导→搜索→下载→阅读）

### 验收标准（Phase A：无账号本地阅读闭环）
- [x] 用户能从设备/文件管理器分享文件到 Readflow（代码入口 + 单测已完成；真机 smoke 待补）
- [x] 导入的书显示在书架上
- [x] 点击书籍进入阅读器
- [x] 阅读进度本地保存并在重启后恢复
