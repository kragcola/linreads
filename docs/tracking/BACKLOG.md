...
> ⛔ **非实时 backlog**。本文件是 2026-06 前后 v1/v2/v3 架构审计与根因裁决的**历史 resolved ledger**（条目多为当时已勾选关闭的设计/构建矛盾）。
> **现网恢复与未完成工作**以 [ACTIVE.md](ACTIVE.md) 为准；Android 架构以 [android-architecture-v4.md](../android-architecture-v4.md) 为准（22 模块、原生 EPUB、PdfRenderer）。
> 文中「21 模块」「epub-ts 默认引擎」等表述反映**当时 v2/v3 裁决语境**，不是 2026-07 当前实现。
> 最后台账整理：2026-06-18（根因裁决后）；本页角色标注：2026-07-19
...
- [x] **ARCHITECTURE.md 更新** — 历史项：当时待指向 v2；**现网**已由 `docs/architecture.md` 指向 v4（2026-07-19 文档收敛），不再作为 live 待办

---

_历史条目规模（归档计数，非 live 未处理数）：约 41 项当时已裁决/勾选的审计发现（P0: 6 / P1: 6 / P2: 6 / P3: 9 / 新发现: 12 / 技能: 4）。**不要**把本计数当作当前未解决问题数。_
