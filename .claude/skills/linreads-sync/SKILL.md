...
3. 无服务器降级：同一局域网内直接用 Calibre Server 的自定义字段存储进度
   GET /ajax/book/<id>/calibre-library → 读 #LinReads_position 自定义列
...
```typescript
// 存储进度
const store = await relationalStore.getRdbStore(context, { name: 'LinReads.db', ... })
...
```typescript
const db = await openDB('LinReads', 1, {
  upgrade(db) {
