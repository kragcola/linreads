...
- 边界：本轮是本地 debug/test APK + AVD UIAutomator/XML/screenshot/tag summary 证据，TalkBack settings 为 `null/0/0`，窄口径 critical grep 只见 1 条无关 Google 输入法 `READ_APP_SPECIFIC_LOCALES` SecurityException，未见 Readflow `FATAL EXCEPTION` / ANR / assertion / OOM / recycled-bitmap。仍不是 release 包、物理平板、人手翻页、TalkBack speech/action-mode、性能 benchmark、真实 SAF 或真实 Calibre 证据；push 后需要用新版 `dev-latest` 再做 release 黑盒，`PAGE-05` 保持 `PARTIAL`
