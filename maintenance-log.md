...
- LinReads PDF：同源 PDF `content://media/external/file/462` 冷启动 `TotalTime=2021ms`，截图显示 `RFMR PDF PAGE 001`，XML 暴露 `阅读内容，捏合缩放页面` 与 `第 1 页，共 20 页`，PSS `170151KB`，gfx p90/p95 `450/600ms`；随后 3 次左滑到 `第 4 页，共 20 页`，page contact sheet 显示 `RFMR-PDF-001..004`，采样后 PSS `168642KB`、gfx p90/p95 `113/121ms`，无 app-level fatal/ANR/OOM/recycled-bitmap grep。结论边界：AVD 小样本里 LinReads PDF 可读且可翻页，MoonReader 同源 PDF 崩溃；但这不是物理平板、真实 DocumentsUI、真实 TalkBack speech 或 release-vs-release 性能结论
