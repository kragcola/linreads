...
- 证据：已从设备拉取到 `/tmp/readflow-stage-verify-20260626/`；A-03 摘要记录书卡 `打开 a03-accessibility-*`、菜单 `<title> 的菜单`、reader surface `阅读内容，捏合调整字号`、action `显示或隐藏阅读工具栏`、chrome/TOC/search/bookmark/annotation/font/theme 标签，且当前 AVD `enabled_accessibility_services=com.google.android.marvin.talkback/com.google.android.marvin.talkback.TalkBackService`、`accessibility_enabled=1`、`touch_exploration_enabled=1`；SAF 摘要记录 `CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 均命中、ZIP `format=linreads-backup` / `schema_version=1`、恢复后进度/删除态书签/标注回灌；Calibre 摘要记录 bad port 失败提示、host-only probe 成功到 `10.0.2.2:8081`、cover event、download event、离线打开正文、移除下载后 `NOT_DOWNLOADED/localUri=null`
