...
- LinReads release 同源黑盒：EPUB clear-data `ACTION_VIEW` 冷启动 `TotalTime=1064ms` / `WaitTime=1069ms`，XML 暴露 `RFMR Chapter 1 Invisible Spacer Stress`、`RFMR-CJK-001..027` 和 reader surface `阅读内容，捏合调整字号`，`blocked_dialog=False`，PSS `65183KB`。可见 `排版 -> 分页` 后采样 pages `0..25`，page 0/4 为章节标题，CJK micro 正文页 packed `20/20/8`，后续混合样式到 `RFMR-MIX-053`；未复现 CJK 一句一页，采样后 PSS `56643KB`、gfx p90/p95 `97/105ms`
- LinReads TXT/PDF release：TXT clear-data `ACTION_VIEW` 冷启动 `TotalTime=904ms`，XML 暴露 `RFMR-TXT-000001..000028`，PSS `60306KB`，gfx p90/p95 `450/450ms`，无阻断弹窗。PDF clear-data `ACTION_VIEW` 冷启动 `TotalTime=1223ms`，XML 暴露 `阅读内容，捏合缩放页面` 和 `第 1 页，共 20 页`；三次左滑到 `第 4 页，共 20 页`，first-page PSS `90979KB`，after-page PSS `96976KB`，after-page gfx p90/p95 `133/150ms`。本轮 PDF 未见 MoonReader 同源 PDF 的崩溃路径
- staged 可模拟项（debug/test APK，不能和 release 黑盒混算）：`./gradlew -Preadflow.phase=2 :app:compileDebugAndroidTestKotlin :app:installDebug :app:installDebugAndroidTest` 通过。A-02 `A02PerformanceRuntimeSmokeTest` = `OK (1 test)`，证据 `staged-smoke/pulled/a02-performance-runtime-smoke`，TXT/EPUB/PDF 首屏代理 `8514/9999/6799ms`、total PSS `225492/252237/303295KB`、PDF `gfx_p90_ms=200`，继续是 AVD instrumentation proxy。A-03 批量串跑先因导入 DB row 时序失败，单独复跑 `OK (1 test)`，summary 覆盖书卡/菜单、reader surface action、chrome、TOC/search/bookmark/annotation/font/theme 标签；本轮 TalkBack settings 为 `null/0/0`，只声明 XML/action 代理，不声明真人 speech。SAF 批量串跑先未等到导出成功文案，单独复跑 `OK (1 test)`，`CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 各 1 次，ZIP schema v1 与恢复回灌通过
- Calibre staged 模拟：fake server 单测 `python3 -m unittest android/test-tools/test_fake_calibre_server.py` = `OK`。`python3 android/test-tools/fake_calibre_server.py --host 127.0.0.1 --port 18181` + `adb reverse tcp:18181 tcp:18181` 后，`CalibreGroupedRuntimeSmokeTest` = `OK (2 tests)`，summary 记录 explicit URL 连接成功、cover event、download event、离线打开、移除下载后 `NOT_DOWNLOADED/localUri=null` 且文件删除。`CalibreDownloadFailureRuntimeSmokeTest` 首跑失败根因为 grouped smoke 已调用 `/__shutdown__` 关闭 fake server；重启 server 后单独复跑 = `OK (1 test)`，服务下线后 `bookRowAfterFailure=null`、`orphanFilesAfterFailure=`、书架仍为空
- 边界：本轮把 LinReads release 同源 S1/S2/S6 补齐，并把 VERIFY/PARTIAL 可模拟项阶段性复跑；但仍不是最终横评结论。缺口仍包括同源 S3/S4/S5/S7/S8、MoonReader 对应未跑场景、物理平板/真实文件管理器/真实 DocumentsUI、真人 TalkBack speech/focus traversal、真实设备性能 benchmark、真实 Calibre LAN/认证

### MoonReader vs LinReads shared-corpus AVD partial comparison
- 范围：继续推进“静读天下 vs LinReads 功能层模拟对比”目标；当前仍只有 `emulator-5554` / `sdk_gphone64_arm64` 在线，tablet-like override `1600x2560` density `320`，不声明真实平板。证据根：`/tmp/readflow-moonreader-linreads-compare-20260627/`；共享语料根：`/tmp/readflow-moonreader-linreads-corpus/`
- MoonReader EPUB：`file://` 打开 `/sdcard/Download/rfmr/rf-mr-extreme-20260627.epub` 失败，logcat 为 `FileNotFoundException ... EACCES (Permission denied)`。改用 MediaStore `content://media/external/file/460` 后，`am start -W` 进入 `ActivityTxt`，`TotalTime=1566ms` / `WaitTime=1611ms`；首次内容被 MoonReader `ClickTip`（`Page Up` / `Options` / `Page Down`）和 Android immersive `Got it` 依次阻断，dismiss 后截图可见 `RFMR Chapter 1 Invisible Spacer Stress` 与 `RFMR-CJK-001..005`
- MoonReader EPUB 翻页：右侧 tap 10 次截图采样显示 `RFMR-CJK-001..048`、`RFMR-MIX-001..068`，未观察到空白页或一短句一页；清空 logcat 后补 3 次 tap 到 `RFMR-TAIL-001..023`。干净窗口 PSS `73453KB`，gfx p90/p95 `81/81ms`，filtered logcat 未见 MoonReader fatal/ANR/OOM，仅早前打开窗口存在 `ActivityMain` `WindowLeaked` 风险
- MoonReader TXT/PDF：TXT `content://media/external/file/461` 冷启动 `TotalTime=1579ms`，首屏可见 `RFMR-TXT-000001..000020`，PSS `64820KB`，但仍有 `WindowLeaked`。PDF `content://media/external/file/462` 冷启动先报 `ActivityTxt TotalTime=1883ms`，随后崩溃回 launcher，logcat 有 `FATAL EXCEPTION: GoldenBoot` / `NullPointerException: println needs a message` / `Process com.flyersoft.moonreaderp ... has died`；同源 PDF 本地识别为 PDF 1.4 / 20 pages，因此作为本轮 MoonReader S1 PDF 失败证据保留
- LinReads 同源语料：本轮使用 AVD 已安装的 local debug/current HEAD `25ba892`（不是重新下载的 release APK）。清数据 EPUB `content://media/external/file/460` 冷启动 `TotalTime=1979ms`，XML/截图暴露 `RFMR-CJK-001..027`，PSS `146899KB`，gfx p90/p95 `250/550ms`，无 app-level fatal/ANR/OOM/recycled-bitmap grep。TXT `content://media/external/file/461` 冷启动 `TotalTime=2593ms`，XML/截图暴露 `RFMR-TXT-000001..000028`，PSS `137682KB`，gfx p90/p95 `150/550ms`，无 app-critical grep
- LinReads PDF：同源 PDF `content://media/external/file/462` 冷启动 `TotalTime=2021ms`，截图显示 `RFMR PDF PAGE 001`，XML 暴露 `阅读内容，捏合缩放页面` 与 `第 1 页，共 20 页`，PSS `170151KB`，gfx p90/p95 `450/600ms`；随后 3 次左滑到 `第 4 页，共 20 页`，page contact sheet 显示 `RFMR-PDF-001..004`，采样后 PSS `168642KB`、gfx p90/p95 `113/121ms`，无 app-level fatal/ANR/OOM/recycled-bitmap grep。结论边界：AVD 小样本里 LinReads PDF 可读且可翻页，MoonReader 同源 PDF 崩溃；但这不是物理平板、真实 DocumentsUI、真实 TalkBack speech 或发布包性能结论（后续 `Dev build #124` release 同源复跑见上方）

### MoonReader vs LinReads extreme reading comparison protocol
- 范围：推进“静读天下 vs LinReads 功能层模拟对比”目标，先落同源语料与对比协议，不声明 MoonReader 黑盒结果。当前设备检查仍只有 `emulator-5554`，无物理平板在线
- 新增共享语料生成器：`android/test-tools/extreme_reading_corpus.py` 输出 `rf-mr-extreme-20260627.epub` / `rf-mr-extreme-20260627-utf8.txt` / `rf-mr-extreme-20260627.pdf` / `manifest.json` / `README.md`。EPUB 覆盖 invisible spacer-only 段、中文极短句、混合 list/blockquote/ruby/br/pre/table、长无空格中文、尾句和短 later spines；TXT 覆盖长 UTF-8、硬换行、near-tail anchor、emoji/grapheme 和 invisible spacer probe；PDF 覆盖 20 页稳定 marker
- 新增 guard：`android/test-tools/test_extreme_reading_corpus.py` 校验 EPUB zip 结构、TXT/PDF markers 和 manifest，防止后续破坏共同语料格式
- 新增协议文档：`docs/research/moonreader-linreads-extreme-reading-comparison.md` 定义同一设备层、同一 launch route、同一指标的 S1-S8 场景矩阵（冷启动、EPUB 极端分页、低视力排版、模式锚点、手势、PDF、搜索/标注、TalkBack），并引用本地 MoonReader 反编译依据：`ActivityTxt` 阅读入口/手势/字号、`MRTextView` 自定义排版、`A.setLineSpace()`、`PDFReader` 独立 PDF 路径和 manifest `ACTION_VIEW` 支持
- 边界：本次是协议与可复现语料层推进，不是最终横向结论；LinReads 仍复用既有 dev122/dev123 AVD 证据作当前基线，MoonReader 尚未跑同一语料黑盒，物理平板真实阅读体验、性能、TalkBack speech/focus 和文件管理器入口仍待补

### Android v4 Dev build #122 release black-box + staged VERIFY/PARTIAL rerun
- 范围：`fix(android): ignore invisible EPUB spacer paragraphs` 已推送并由 GitHub Actions 发布 `dev-latest` / `Dev build #122` / commit `956e93fa54bbe42db4cad8395101605e63911ca5`。下载远端 `app-ota.apk` 到 `/tmp/readflow-dev122-release-check/app-ota.apk`（大小 `9,729,356`，SHA-256 `c39d5b015f3796ad012a565b77e8844ea37ee05af243c61dd4d88ede85d69e87`），安装到唯一在线 `emulator-5554` / `sdk_gphone64_arm64` / tablet-like override `1600x2560` density `320`
- release 黑盒：新建 `/tmp/readflow-dev122-release-blackbox-20260627/dev122-invisible-spacer-extreme.epub`，覆盖 7 类 invisible spacer-only 段、48 个中文极短句、90 个英文 micro 段、1 个长无空格中文段和 24 个 tail 短段。通过 MediaStore `content://media/external/file/428` ACTION_VIEW 冷启动，初次 `TotalTime=1142ms`，强停重开 `TotalTime=869ms`；首屏 XML 暴露 `DEV122` 与 reader surface，未出现通知/安装权限阻断。连续模式首屏直接可见 27 个 `DEV122 隐形短句`，证明 spacer-only 段未占可见正文。经用户可见 `排版 -> 分页` 后采样 25 次，正文前 8 个内容采样真实短块数 `20/20/20/20/20/20/20/18`，长中文段和 tail 页均可见；`blocked_dialog_seen=false`，`release_window_devreadflow_critical_grep_lines=0`。证据 `/tmp/readflow-dev122-release-blackbox-20260627/dev122-release-blackbox-summary.txt`
- staged AVD 可模拟项：证据总表 `/tmp/readflow-stage-verify-20260627-dev122/staged-verification-summary.md`。`A02PerformanceRuntimeSmokeTest` = `OK (1 test)`，TXT/EPUB/PDF 首屏代理 `8110/10416/6922ms`、total PSS `213402/250310/301655KB`、PDF `gfx_p90_ms=150`，dev.readflow critical grep 0，继续是 noisy AVD proxy。TalkBack settings-on AVD `A03AccessibilityRuntimeSmokeTest` = `OK (1 test)`，settings 期间 `enabled_accessibility_services=TalkBack`、`accessibility_enabled=1`、`touch_exploration_enabled=1`，结束恢复 `null/0/0`；summary 覆盖书卡/菜单、reader surface action、chrome、TOC/search/bookmark/annotation/font/theme 标签，dev.readflow critical grep 0
- SAF/Calibre：`BackupSafUiRuntimeSmokeTest` = `OK (1 test)`，`CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 各 1 次，ZIP `manifest_format=linreads-backup`、`manifest_schema=1`、恢复后进度/删除态书签/标注回灌，dev.readflow critical grep 0。Calibre fake server 使用 `127.0.0.1:18181` + `adb reverse`，`CalibreGroupedRuntimeSmokeTest` = `OK (2 tests)`，显式 URL 连接、cover/download event、离线打开、移除下载清理均通过；重启 server 后 `CalibreDownloadFailureRuntimeSmokeTest` = `OK (1 test)`，服务下线后 `bookRowAfterFailure=null`、`orphanFilesAfterFailure=`、书架仍为空，两组 dev.readflow critical grep 均为 0
- 边界：本轮仍只有 AVD release APK / AVD instrumentation / fake server / test ContentProvider / ActivityMonitor 证据，不是物理平板、真实 OEM 文件管理器/分享面板、真人 TalkBack speech/focus traversal、真实设备性能 benchmark、真实 DocumentsUI 或真实 Calibre LAN/认证；`A-01/SRC-01~09` 保持 `VERIFY`，`A-02/A-03/PAGE-05` 保持 `PARTIAL`

### Android v4 PAGE-05 invisible spacer EPUB hardening
- 根因/修复：继续按“产品实际测试工程师”口径补网页抓取/Calibre 转换 EPUB 噪声。RED `EpubReaderItemParserTest.invisible spacer paragraphs are ignored` 证明只有不可见 spacer 的 `<p>` 会漏入 reader items：`&nbsp;`、全角空格、zero-width space、BOM/zero-width no-break space、word joiner、ZWNJ/ZWJ 等肉眼不可见段可能制造伪空白正文/分页噪声。修复 `EpubReaderItemParser.trimTextWithRanges()` 的边缘裁剪判定，除 `Character.isWhitespace` 外同时处理 `Character.isSpaceChar` 和常见 invisible spacer chars，仅裁掉段首/段尾 spacer，不全局删除正文中字符
- JVM 验证：RED 后最小修复，targeted `./gradlew -Preadflow.phase=2 :render:epub:testDebugUnitTest --tests "dev.readflow.render.epub.EpubReaderItemParserTest.invisible spacer paragraphs are ignored"` 通过；parser 全量 `EpubReaderItemParserTest` 通过；PAGE-05 相关 reflow targeted（相邻短段、中文对话微短段、lazy later spines）通过；`EpubPageMappingTest` 全量通过；`git diff --check` 通过
- AVD reader-route 验证：新增 `EpubPagedRuntimeSmokeTest.epubPagedIgnoresInvisibleSpacerParagraphsAndPacksTinyCjkRuntime`，在唯一在线 `emulator-5554` / `sdk_gphone64_arm64` / tablet-like override `1600x2560` density `320` 上安装 debug/test APK，清空旧 evidence 后 targeted instrumentation = `OK (1 test)`。证据 `/tmp/readflow-page05-invisible-spacers-20260627-clean/page05-epub-runtime-smoke/packed-invisible-spacers-summary.txt` 记录 48 个真实中文短句、7 类 invisible spacer 段，`page_count=3`、采样页真实短句数 `20/20/8`，没有空白页或一句一页；截图/XML 共 7 个文件已拉取
- 边界：本轮本身是本地 debug/test APK + AVD UIAutomator/XML/screenshot/tag summary 证据，TalkBack settings 为 `null/0/0`，窄口径 critical grep 只见 1 条无关 Google 输入法 `READ_APP_SPECIFIC_LOCALES` SecurityException，未见 Readflow `FATAL EXCEPTION` / ANR / assertion / OOM / recycled-bitmap。后续已由 `Dev build #122` release 黑盒补证，但仍不是物理平板、人手翻页、TalkBack speech/action-mode、性能 benchmark、真实 SAF 或真实 Calibre 证据；`PAGE-05` 保持 `PARTIAL`

### Android v4 Dev build #120 release black-box + staged VERIFY/PARTIAL rerun
- 范围：继续按“产品实际测试工程师”口径复验当前平板可更新包和 AVD 可模拟缺口，不改产品代码。`dev-latest` 已是 `Dev build #120` / commit `5fd6585`；下载远端 `app-ota.apk` 到 `/tmp/readflow-dev120-release-blackbox-20260627/app-ota.apk`（大小 `9,729,356`，SHA-256 `707e127831b2b7f625ca6425b377e88affb51d9317fb1448752fa4afb3e47893`），安装到 tablet-like `emulator-5554`（`1600x2560` / density `320`），通过 MediaStore `content://media/external/file/427` ACTION_VIEW 打开 `/tmp/readflow-dev120-release-blackbox-20260627/dev120-extreme.epub`
- release 黑盒：默认/可见分页路径冷启动 `TotalTime=1191ms`，强停重开 `TotalTime=1463ms`；首屏暴露 `DEV120` 与 reader surface，未出现通知/安装权限阻断。经用户可见 `排版 -> 分页` 后采样 30 页，正文页 29 页，短块数 min/max `5/20`，29 页均有 2+ 个短块。随后通过可见 `排版` sliders 调到 `30sp` / `2.20x` 低视力场景再采样 18 页，正文页 15 页，短块数 min/max `2/9`，15 页均有 2+ 个短块；长中文段跨页但未退回“一句一页”。`blocked_dialog_seen=false`，过滤到 `dev.readflow` 的 fatal/ANR/security/file/OOM/assertion/recycled-bitmap grep 为 0
- PAGE-05 精确补证：targeted `EpubPagedRuntimeSmokeTest#epubPagedPacksMicroParagraphsWithMaxTypographyInLandscapeRuntime` = `OK (1 test)`，证据 `/tmp/readflow-stage-verify-20260627-dev120/page05/page05-epub-runtime-smoke/packed-max-typography-summary.txt` 记录 `paragraph_count=72`、`persisted_font_size_sp=32`、`persisted_line_spacing=2.2`、landscape `2560x1600`、`landscape_page_count=15`，第 1/2 页各 5 个 `A11y line`，不是 72 页一行一页；PAGE-05 仍保持 `PARTIAL`，因为这仍非物理平板/真人手势/TalkBack/性能证据
- staged AVD 可模拟项：证据总表 `/tmp/readflow-stage-verify-20260627-dev120/staged-verification-summary.md`。`A02PerformanceRuntimeSmokeTest` = `OK (1 test)`，TXT/EPUB/PDF 首屏代理 `7833/9040/6606ms`、total PSS `223537/241235/304466KB`、PDF `gfx_p90_ms=150`，critical grep 0，继续为 noisy AVD proxy；`A03AccessibilityRuntimeSmokeTest` 输出 `OK (1 test)`，标签/action 覆盖书卡/菜单、reader surface、chrome、TOC/search/bookmark/annotation/font/theme，critical grep 0，但本轮 `touch_exploration_enabled` 抓取为 `0`，不声明真实触摸探索或 TalkBack speech
- SAF/Calibre：`BackupSafUiRuntimeSmokeTest` = `OK (1 test)`，`CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 各 1 次，ZIP `manifest_format=linreads-backup`、`manifest_schema=1`、恢复后进度/删除态书签/标注回灌，critical grep 0。Calibre fake server 使用 `127.0.0.1:18181` + `adb reverse`，`CalibreGroupedRuntimeSmokeTest` = `OK (2 tests)`，显式 URL 连接、cover/download event、离线打开、移除下载清理均通过；重启 server 后 `CalibreDownloadFailureRuntimeSmokeTest` = `OK (1 test)`，服务下线后 `bookRowAfterFailure=null`、`orphanFilesAfterFailure=`、书架仍为空，critical grep 0
- 边界：本轮仍只有 AVD release APK / AVD instrumentation / fake server / test ContentProvider / ActivityMonitor 证据，不是物理平板、真实 OEM 文件管理器/分享面板、真人 TalkBack speech/focus traversal、真实设备性能 benchmark、真实 DocumentsUI 或真实 Calibre LAN/认证；`A-01/SRC-01~09` 保持 `VERIFY`，`A-02/A-03/PAGE-05` 保持 `PARTIAL`

### Android v4 Dev build #118 release black-box + staged VERIFY/PARTIAL rerun
- 范围：继续按“产品实际测试工程师”口径复验当前平板可更新包和 AVD 可模拟缺口，不改产品代码。`dev-latest` 已是 `Dev build #118`；下载远端 `app-ota.apk` 到 `/tmp/readflow-dev118-release-blackbox-20260627/app-ota.apk`（大小 `9,729,356`，SHA-256 `f540011443271cfe36b16fe10142ba2bb2bdf510a7d9a1cd25507706274a8bd2`），安装到 `emulator-5554`，`pm clear dev.readflow` 后通过 MediaStore `content://media/external/file/426` ACTION_VIEW 冷启动 `/tmp/readflow-dev118-release-blackbox-20260627/dev118-extreme-reader.epub`，启动 `TotalTime=1056ms`
- release 黑盒：样本覆盖 120 个混合微短段、6 个后续短 chapter spine、48 个无空格中文长句片段和 24 个尾句短段。首屏 `cold-start.xml` 暴露 `DEV118` 与 reader surface `阅读内容，捏合调整字号`，未出现通知/安装权限阻断。经用户可见 `排版 -> 分页` 切换后采样 39 页，`dev118-release-blackbox-summary.txt` 记录 `max_micro_markers_on_page=14`、`max_short_markers_on_page=10`、`max_long_markers_on_page=48`、`max_tail_markers_on_page=20`、`min_positive_markers_on_page=4`、`pages_with_2plus_markers=30`、`long_seen=true`、`tail_seen=true`、`blocked_dialog_seen=false`、`logcat_app_critical_grep_lines=0`；#118 发布包未复现“一句一页”或首读权限弹窗阻断
- staged AVD 可模拟项：证据根 `/tmp/readflow-stage-verify-20260627-dev118/staged-verification-summary.md`。`A01IncomingBookRuntimeSmokeTest` = `OK (1 test)`，50 个 VIEW/SEND × TXT/MD/EPUB/PDF 入口证据文件；`A02PerformanceRuntimeSmokeTest` = `OK (1 test)`，TXT/EPUB/PDF 首屏代理 `7673/9861/7554ms`、total PSS `214063/251273/303512KB`、TXT/EPUB/PDF `gfx_p90_ms=65/48/150`，继续是 noisy AVD proxy；TalkBack settings-on AVD `A03AccessibilityRuntimeSmokeTest` = `OK (1 test)`，summary 记录书卡、reader surface action、chrome、TOC/search/bookmark/annotation/font/theme 标签，测试后 settings 已恢复 `null/0/0`
- SAF/Calibre：`BackupSafUiRuntimeSmokeTest` = `OK (1 test)`，`CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 各命中 1 次，ZIP `manifest_format=linreads-backup`、`manifest_schema=1`、`manifest_counts=progress:1,bookmarks:1,annotations:1`，恢复后进度/删除态书签/标注回灌。Calibre fake server 使用 `127.0.0.1:18181` + `adb reverse`，`CalibreGroupedRuntimeSmokeTest` = `OK (2 tests)`，显式 URL 连接、cover/download event、离线打开、移除下载清理均通过；`CalibreDownloadFailureRuntimeSmokeTest` 首次因 grouped smoke 已关闭共享 fake server 而在连接阶段失败，重启 fake server 后单独复跑 = `OK (1 test)`，服务下线后 `bookRowAfterFailure=null`、`orphanFilesAfterFailure=`、书架仍为空；SAF/Calibre app-critical logcat grep 均为 0
- 边界：本轮仍只有 AVD release APK / AVD instrumentation / fake server / test ContentProvider / ActivityMonitor 证据，不是物理平板、真实 OEM 文件管理器/分享面板、真人 TalkBack speech/focus traversal、真实设备性能 benchmark、真实 DocumentsUI 或真实 Calibre LAN/认证；`A-01/SRC-01~09` 保持 `VERIFY`，`A-02/A-03/PAGE-05` 保持 `PARTIAL`

### Android v4 Dev build #117 release black-box + staged VERIFY/PARTIAL rerun
- 范围：继续按“产品实际测试工程师”口径复验当前平板可更新包和 AVD 可模拟缺口，不改产品代码。`dev-latest` 已是 `Dev build #117` / commit `7203d214d8a5f2ee3cef8d5cfdcdf1f9c15e6719`；下载远端 `app-ota.apk` 到 `/tmp/readflow-dev117-release-blackbox-20260627/app-ota.apk`（大小 `9,729,356`，SHA-256 `f24bfa8d4df55ad89f26c1ddd5c1a45f31c95128a5d4b8b5b0fd2471b919d88c`），安装到 `emulator-5554`，`pm clear dev.readflow` 后通过 MediaStore `content://media/external/file/425` ACTION_VIEW 冷启动 `/tmp/readflow-dev117-release-blackbox-20260627/dev117-extreme-reader.epub`，启动 `TotalTime=1187ms`
- release 黑盒：样本覆盖 120 个混合微短段、6 个后续短 chapter spine、48 个无空格中文长句片段和 24 个尾句短段。首屏 `cold-start.xml` 暴露 `DEV117` 与 reader surface `阅读内容，捏合调整字号`，未出现通知/安装权限阻断。经用户可见 `排版 -> 分页` 切换后扩展采样 39 页，`dev117-release-blackbox-summary.txt` 记录 `max_micro_markers_on_page=6`、`max_short_markers_on_page=10`、`max_long_markers_on_page=48`、`max_tail_markers_on_page=20`、`pages_with_2plus_markers=30`、`long_seen=true`、`tail_seen=true`、`blocked_dialog_seen=false`、`logcat_critical_grep_lines=0`；#117 发布包未复现“一句一页”或首读权限弹窗阻断
- staged AVD 可模拟项：证据根 `/tmp/readflow-stage-verify-20260627-dev117/staged-verification-summary.md`。`A01IncomingBookRuntimeSmokeTest` = `OK (1 test)`，50 个 VIEW/SEND × TXT/MD/EPUB/PDF 入口证据文件，logcat critical grep 0；`A02PerformanceRuntimeSmokeTest` = `OK (1 test)`，TXT/EPUB/PDF 首屏代理 `7547/9184/7107ms`、total PSS `221046/251627/303479KB`、PDF `gfx_p90_ms=150`，logcat grep 只见 2 行无关 `com.google.android.gms` `SecurityException`；TalkBack settings-on AVD `A03AccessibilityRuntimeSmokeTest` = `OK (1 test)`，summary 记录书卡、reader surface action、chrome、TOC/search/bookmark/annotation/font/theme 标签，测试后 settings 已恢复 `null/0/0`
- SAF/Calibre：`BackupSafUiRuntimeSmokeTest` = `OK (1 test)`，`CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 各命中 1 次，ZIP `manifest_format=linreads-backup`、`manifest_schema=1`、`manifest_counts=progress:1,bookmarks:1,annotations:1`，恢复后进度/删除态书签/标注回灌。Calibre fake server 使用 `127.0.0.1:18181` + `adb reverse`，`CalibreGroupedRuntimeSmokeTest` = `OK (2 tests)`，显式 URL 连接、cover/download event、离线打开、移除下载清理均通过；`CalibreDownloadFailureRuntimeSmokeTest` = `OK (1 test)`，服务下线后 `bookRowAfterFailure=null`、`orphanFilesAfterFailure=`、书架仍为空
- 边界：本轮仍只有 AVD release APK / AVD instrumentation / fake server / test ContentProvider / ActivityMonitor 证据，不是物理平板、真实 OEM 文件管理器/分享面板、真人 TalkBack speech/focus traversal、真实设备性能 benchmark、真实 DocumentsUI 或真实 Calibre LAN/认证；`A-01/SRC-01~09` 保持 `VERIFY`，`A-02/A-03/PAGE-05` 保持 `PARTIAL`
...
- dev114 staged rerun：证据根 `/tmp/readflow-stage-verify-20260627-dev114/`。A-02 `A02PerformanceRuntimeSmokeTest` = `OK (1 test)`，TXT/EPUB/PDF 首屏代理 `8408/10136/7407ms`、total PSS `213956/234403/302807KB`、PDF `gfx_p90_ms=150`，继续 `PARTIAL`；A-03 TalkBack settings-on AVD `A03AccessibilityRuntimeSmokeTest` = `OK (1 test)`，summary 记录书卡/菜单、reader surface action、chrome、TOC/search/bookmark/annotation/font/theme 标签，但仍不是真人 TalkBack speech/focus traversal，继续 `PARTIAL`
- SAF/Calibre 模拟复跑：`BackupSafUiRuntimeSmokeTest` = `OK (1 test)`，summary 记录 `CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 各 1 次、ZIP `format=linreads-backup` / `schema_version=1`、恢复后进度/删除态书签/标注回灌；`CalibreGroupedRuntimeSmokeTest` 在 `127.0.0.1:18181` + `adb reverse` 下 = `OK (2 tests)`，`CalibreDownloadFailureRuntimeSmokeTest` = `OK (1 test)`，cover/download/offline/remove/failure-cleanup 均有证据。边界：均为 AVD + fake server / ActivityMonitor / test ContentProvider，不是真实 Calibre LAN/认证、真实 DocumentsUI/OEM picker 或物理平板证据；`SRC-01~04/SRC-06~09` 保持 `VERIFY`
...
- 验证：`./gradlew -Preadflow.phase=2 :app:compileDebugAndroidTestKotlin :app:installDebug :app:installDebugAndroidTest` 通过；启动 `python3 android/test-tools/fake_calibre_server.py --host 127.0.0.1 --port 18181`，并通过 `adb -s emulator-5554 reverse tcp:18181 tcp:18181` 让 app 访问 `http://127.0.0.1:18181`
- grouped 复跑：`adb -s emulator-5554 shell am instrument -w -e class dev.readflow.src01.CalibreGroupedRuntimeSmokeTest -e calibreBaseUrl http://127.0.0.1:18181 -e calibreHostHint 127.0.0.1 dev.readflow.test/androidx.test.runner.AndroidJUnitRunner` = `OK (2 tests)`；证据 `/tmp/readflow-calibre-param-20260627/grouped/calibre-runtime-smoke` 记录 explicit URL 连接成功、cover event、download event、离线打开正文、移除下载后 `NOT_DOWNLOADED/localUri=null` 且下载文件删除；logcat critical grep 为 0
- failure 复跑：`adb -s emulator-5554 shell am instrument -w -e class dev.readflow.src06.CalibreDownloadFailureRuntimeSmokeTest -e calibreBaseUrl http://127.0.0.1:18181 dev.readflow.test/androidx.test.runner.AndroidJUnitRunner` = `OK (1 test)`；证据 `/tmp/readflow-calibre-param-20260627/failure/calibre-download-failure-runtime-smoke/download-failure-summary.txt` 记录 `serverUnavailableBeforeDownload=true`、`bookRowAfterFailure=null`、`orphanFilesAfterFailure=`，书架仍是 `还没有书` / `连接 Calibre，或导入本地文件`；logcat critical grep 为 0
- 边界：这是 AVD + fake Calibre server + `adb reverse` 模拟，用于恢复可重复验收能力；仍不是真实 Calibre Content Server、真实 LAN、认证服务器、物理平板/手机、真实封面视觉或大文件下载体验。`SRC-01~04/SRC-06/07` 继续保持 `VERIFY`

### Android v4 late staged verification after #110 release
- 范围：#110 release 黑盒之后继续推进 VERIFY/PARTIAL 中当前环境可模拟的项；`adb devices -l` 仍只发现 `emulator-5554` / `sdk_gphone64_arm64`，没有真实手机/平板在线。本轮不把 AVD、UIAutomator、ActivityMonitor、fake server 或 release shell launch 冒充为物理设备、真实 TalkBack speech、真实 DocumentsUI 或真实 Calibre LAN
- A-01：`A01IncomingBookRuntimeSmokeTest` 串行复跑 `OK (1 test)`，覆盖 ACTION_VIEW/ACTION_SEND × TXT/MD/EPUB/PDF 共 8 个 case，证据 `/tmp/readflow-stage-verify-20260627-late/a01/a01-incoming-runtime-smoke/a01-incoming-summary.txt`，拉取 50 个截图/XML/DB 文件；logcat critical grep 为 0 行。边界仍是 AVD + test external ContentProvider，不是真实文件管理器/分享面板/DocumentsUI
- A-03：临时写入 AVD accessibility settings（`enabled_accessibility_services=com.google.android.marvin.talkback/...`、`accessibility_enabled=1`）后复跑 `A03AccessibilityRuntimeSmokeTest` = `OK (1 test)`，证据 `/tmp/readflow-stage-verify-20260627-late/a03-talkback-on/a03-accessibility-runtime-smoke/a03-accessibility-summary.txt` 记录书卡、reader surface action、chrome、TOC/search/bookmark/annotation/font/theme 标签；logcat critical grep 为 0 行。重要边界：本轮 `touch_exploration_enabled` 未保持为 1（记录为 `0 -> 0 -> 0`），最终 `dumpsys accessibility` 为 `touchExplorationEnabled=false`，因此不能声明真人 TalkBack speech 或完整触摸探索遍历
- A-02：`A02PerformanceRuntimeSmokeTest` = `OK (1 test)`，证据 `/tmp/readflow-stage-verify-20260627-late/a02/a02-performance-runtime-smoke/a02-performance-summary.txt` 记录 TXT/EPUB/PDF 首屏代理 `7940/10040/6722ms`、total PSS `222109/247058/301428KB`、PDF `gfx_p90_ms=150`；logcat grep 只见一条无关 GMS `GoogleApiManager` / `com.google.android.gms` `SecurityException`，无 `dev.readflow` crash/ANR/recycled-bitmap/reader surface/assertion 失败。A-02 继续 `PARTIAL`
- SAF：`BackupSafUiRuntimeSmokeTest` = `OK (1 test)`，证据 `/tmp/readflow-stage-verify-20260627-late/saf/backup-saf-ui-runtime-smoke/backup-saf-ui-summary.txt` 记录 `CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 命中、ZIP `format=linreads-backup` / `schema_version=1`、恢复后进度/删除态书签/标注回灌；logcat critical grep 为 0 行。边界仍是 ActivityMonitor + test ContentProvider，不是真实 DocumentsUI/OEM picker
- Calibre：完整 grouped fake-Calibre smoke 本轮最初未复跑成功，因为宿主 8081 被 Docker 容器 `qq-bot` 占用（`0.0.0.0:8081->8080/tcp`），而当时 `CalibreGroupedRuntimeSmokeTest` 硬编码 `http://10.0.2.2:8081`；随后已在同日 follow-up 中把 Calibre runtime smoke 参数化，并用 18181 + `adb reverse` 跑通 grouped/failure，两条新证据见上一节。真实 Calibre LAN/平板 smoke 仍缺
- 总结：台账写入 `/tmp/readflow-stage-verify-20260627-late/staged-verification-summary.md`；本轮推进了 A-01/A-03/A-02/SAF 的模拟证据与边界记录，Calibre 端口问题已由后续参数化 smoke 复验收口；但未改变 A-01/A-03/A-02/SRC-01~09 的 VERIFY/PARTIAL 状态
...
- release #110 复验：推送 commit `79d68323576338c00a8a397ffa9f50f996df3692` 后，GitHub Actions `Android Dev Release` run `28260094056` 发布 `Dev build #110`。下载远端 `dev-latest/app-ota.apk`（大小 `9,729,356`，SHA-256 `95fb7f51cc4889819600267077f03a34923de75672919862506b8ec3e410e174`）并安装到 `emulator-5554`，清数据后通过同一 MediaStore `content://media/external/file/306` ACTION_VIEW 打开 EPUB，冷启动 `TotalTime=1818ms`；`/tmp/readflow-dev110-release-blackbox/page-sampling.txt` 第 1 次采样即记录 `blocked_dialog=false`、`reader_marker=true`，XML 暴露 `阅读内容，捏合调整字号` 与 `DEV107 micro paragraph 000..027`，未出现 `Allow Readflow to send you notifications?`、`需要安装权限`、`前往设置` 或 `暂不`
- 边界：#110 远端 release AVD 黑盒已证明 #109 首读权限弹窗 blocker 不再复现；logcat critical grep 仅命中一条 GMS `GoogleApiManager` / `com.google.android.gms` `SecurityException`，未见 `dev.readflow` crash/ANR/recycled-bitmap/reader surface/assertion 失败。A-01 仍保持 `VERIFY`，因为真实手机/平板、OEM 文件管理器、Android sharesheet、DocumentsUI 或第三方文件 App 尚未完成
...
- release #108 复验：GitHub Actions `Android Dev Release` 发布 `Dev build #108` / commit `cf861e3` 后，下载远端 `app-ota.apk`（大小 `9,729,356`，SHA-256 `08a4e521a39ae83e8bb37d1db4966207fcf145dc3542e3247a8cc250e238cb60`）并安装到 `emulator-5554`；同一 `content://media/external/file/306` EPUB 冷启动 `TotalTime=924ms`。`/tmp/readflow-dev108-extreme-smoke/epub-release-blackbox/page-sampling.txt` 显示 page 11 同页 packed `DEV107 对话001`、`DEV107 清单002`、`DEV107 旁白003` ... `DEV107 清单020` 共 20 个混合 safe-style markers，page 14/15 也分别 packed 20 个 markers，#107 的一页一个短块未复现
- 边界：#108 已补 release APK AVD 黑盒复验，但当前仍只有 `emulator-5554`，没有真实物理平板、人手翻页、真实 TalkBack speech、真实性能 benchmark、真实 Calibre LAN 或真实 SAF DocumentsUI；logcat 关键 grep 只见一条 Android 系统 locale `SecurityException` 警告，未见 Readflow crash/ANR/recycled-bitmap/reader-surface/assertion 失败；`PAGE-05` 保持 `PARTIAL`
...
- A-03：先复跑 `A03AccessibilityRuntimeSmokeTest` = `OK (1 test)`，再临时打开 AVD TalkBack settings（`enabled_accessibility_services=com.google.android.marvin.talkback/...`、`accessibility_enabled=1`、`touch_exploration_enabled=1`）复跑同测也为 `OK (1 test)`；证据 `/tmp/readflow-stage-verify-20260627/a03-talkback-on/a03-accessibility-runtime-smoke/a03-accessibility-summary.txt` 记录书卡/菜单、reader surface action、chrome、TOC/search/bookmark/annotation/font/theme 标签。测试后已恢复 TalkBack settings 为 `null/0/0`
- A-02：`A02PerformanceRuntimeSmokeTest` = `OK (1 test)`；证据 `/tmp/readflow-stage-verify-20260627/a02/a02-performance-runtime-smoke/a02-performance-summary.txt` 记录 TXT/EPUB/PDF 首屏代理 `7839ms/10256ms/7131ms`，total PSS `222554/245131/302291KB`，PDF `gfx_p90_ms=200`。该结果继续证明 AVD instrumentation proxy 噪声和 PDF frame gate 风险存在，`A-02` 不升格
- SAF：`BackupSafUiRuntimeSmokeTest` = `OK (1 test)`；证据 `/tmp/readflow-stage-verify-20260627/saf/backup-saf-ui-runtime-smoke/backup-saf-ui-summary.txt` 记录 `CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 均命中、ZIP `format=linreads-backup` / `schema_version=1`、恢复后进度/删除态书签/标注回灌
- Calibre：启动 `python3 android/test-tools/fake_calibre_server.py --host 127.0.0.1 --port 8081` 后，`CalibreGroupedRuntimeSmokeTest` = `OK (2 tests)`；证据 `/tmp/readflow-stage-verify-20260627/calibre/calibre-runtime-smoke` 记录 bad port 失败提示、host-only probe 成功到 `10.0.2.2:8081`、cover/download events、离线打开正文和移除下载后 `NOT_DOWNLOADED/localUri=null`
- 阶段总结：总证据台账写入 `/tmp/readflow-stage-verify-20260627/staged-verification-summary.md`；logcat 关键 grep 对 release 黑盒、A-02、A-03、TalkBack-on A-03、Calibre、SAF 均未命中 Readflow `FATAL EXCEPTION` / `ANR in dev.readflow` / `data_app_crash` / recycled-bitmap / security permission 等关键失败。状态保持：`A-02/A-03` 仍为 `PARTIAL`，`SRC-01~04/SRC-06~09` 与 `UX-03/UX-04` 仍为 `VERIFY`
...
- 证据：已从设备拉取到 `/tmp/readflow-stage-verify-20260626/`；A-03 摘要记录书卡 `打开 a03-accessibility-*`、菜单 `<title> 的菜单`、reader surface `阅读内容，捏合调整字号`、action `显示或隐藏阅读工具栏`、chrome/TOC/search/bookmark/annotation/font/theme 标签，且该轮 AVD settings 记录 `enabled_accessibility_services=com.google.android.marvin.talkback/com.google.android.marvin.talkback.TalkBackService`、`accessibility_enabled=1`、`touch_exploration_enabled=1`；SAF 摘要记录 `CREATE_DOCUMENT` / `OPEN_DOCUMENT` monitor 均命中、ZIP `format=linreads-backup` / `schema_version=1`、恢复后进度/删除态书签/标注回灌；Calibre 摘要记录 bad port 失败提示、host-only probe 成功到 `10.0.2.2:8081`、cover event、download event、离线打开正文、移除下载后 `NOT_DOWNLOADED/localUri=null`
- 调试记录：曾并行启动 A-03 与 SAF 两个 UI instrumentation，SAF 返回 `Process crashed`；logcat 显示 A-03 正常结束后 instrumentation force-stop 目标进程，未见 app `FATAL EXCEPTION`，SAF 串行重跑通过。结论：这是并行 UIAutomator 抢同一 AVD 的测试调度问题，后续此类 UI runtime smoke 必须串行跑
- 边界：A-03 仍是 AVD instrumentation + XML/截图，不是物理手机/平板、真人完整焦点顺序或 TalkBack speech/TTS；Calibre 仍是 AVD + fake server，不是真实 Calibre Content Server、真实 LAN、认证服务器或真实封面视觉；SAF 仍是 AVD ActivityMonitor + test ContentProvider，不是真实 DocumentsUI/OEM 文件管理器/真人选择；`A-03` 保持 `PARTIAL`，`SRC-01~04/SRC-06~09` 保持 `VERIFY`
...
- 验证：`./gradlew -Preadflow.phase=2 :app:compileDebugAndroidTestKotlin :app:installDebugAndroidTest` 通过；`adb -s emulator-5554 shell am instrument -w -e class dev.readflow.a03.A03AccessibilityRuntimeSmokeTest dev.readflow.test/androidx.test.runner.AndroidJUnitRunner` = `OK (1 test)`；该轮 AVD 运行后 `enabled_accessibility_services=com.google.android.marvin.talkback/com.google.android.marvin.talkback.TalkBackService`、`accessibility_enabled=1`、`touch_exploration_enabled=1`
...
- 边界：这是 AVD instrumentation + UIAutomator/XML/screenshot 证据，虽然该轮 AVD TalkBack/touch exploration 处于开启状态，但没有物理手机/平板、真人完整焦点顺序、TalkBack speech/TTS 人耳确认或真实手势遍历；`A-03` 保持 `PARTIAL`
...
- 证据：`/tmp/readflow-page05-packed-line-spacing-runtime-20260626/full-class/page05-epub-runtime-smoke/packed-micro-summary.txt` 记录 baseline `paragraph_count=36` / `page_count=3` / 第 1 页 14 段；pinch 后 `after_pinch_page_count=4`；行距 slider 拖动后 `after_line_spacing_persisted=2.2`、`after_line_spacing_page_count=5`、第 3 页仍含 `Line 017..024` 8 段；logcat grep 未命中 crash/ANR/recycled-bitmap
- 边界：这是 AVD instrumentation + UIAutomator slider + screenshot/XML/tag summary 证据，不是真实手机/平板手指调参、TalkBack speech/action-mode、跨屏视觉调校或帧率/PSS 预算；`PAGE-05` 保持 `PARTIAL`，`UX-04` 保持 `VERIFY`
...
- 边界：这是 JVM/Robolectric page-budget 与 locator contract 证据，不是真实设备 mixed EPUB 视觉、TalkBack speech/action-mode、Compose selection action-mode、帧率/PSS 内存或 Calibre/SAF 真机 smoke；`PAGE-05` 仍保持 `PARTIAL`，`A-02` 仍保持 `PARTIAL`
