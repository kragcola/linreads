...
- Calibre：完整 grouped fake-Calibre smoke 本轮未复跑成功，因为宿主 8081 已被 Docker 容器 `qq-bot` 占用（`0.0.0.0:8081->8080/tcp`），而 `CalibreGroupedRuntimeSmokeTest` 硬编码 `http://10.0.2.2:8081`。改跑 18081 的 `CalibreDownloadFailureRuntimeSmokeTest` 也在连接成功文案前超时，fake server 日志仅有 host-side curl 预检、未见 app 请求；本轮不新增 Calibre 通过证据，继续沿用 2026-06-26/早前 2026-06-27 的 fake-Calibre baseline，真实 Calibre LAN/平板 smoke 仍缺
- 总结：台账写入 `/tmp/readflow-stage-verify-20260627-late/staged-verification-summary.md`；本轮推进了 A-01/A-03/A-02/SAF 的模拟证据与边界记录，但未改变 A-01/A-03/A-02/SRC-08~09 的 VERIFY/PARTIAL 状态
